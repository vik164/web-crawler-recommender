...
/* ---------------------------FRUIT CRAWLER------------------------------ */
  ...
  let fruitVisited = new Set();

  let fruitCBCount = 0;

  // initialize fruit crawler
  const fruitCrawler = new Crawler({
    maxConnections: 1,
    callback: fruitCrawlerCB
  });

  function fruitCrawlerCB (error, res, done) {  
    if (error) {
      console.log(error);
    } else {
      ...
      let body = $("p").text().replace(/\n/g, ' ');

      fruitCBCount++;
      console.log("CURRENT CRAWLED URL[" + fruitCBCount + "]: " + currentUrl);

      $(links).each(async function(i, link){
        linkUrlText = $(link).text();
        linkUrlHref = $(link).attr('href');
        let linkCompleteUrl = url.resolve(fruitBaseUrl, linkUrlHref);

        // IF LINK HAS BEEN fruitVisited BEFORE
        if(fruitVisited.has(linkCompleteUrl)) {
          const filter = {url: linkCompleteUrl};
          const update = {
            $push: { incomingUrls: currentUrl },
            $inc: { incomingCount: 1 }
          };
          let result = await FruitPage.findOneAndUpdate(filter, update, {new: true});
          if (!result) {
            console.log("LINK PAGE fruitVisited BUT NOT IN DB");
          }
        }

        // IF LINKED HAS NOT BEEN fruitVisited YET
        else{
          let newFruitPage = new FruitPage();
          newFruitPage.url = linkCompleteUrl;
          newFruitPage.incomingCount = 1;
          newFruitPage.outgoingCount = 0;
          let numberMatch = linkCompleteUrl.match(/\d+/);
          let extractedNumber = -1;
          if (numberMatch) {
            extractedNumber = parseInt(numberMatch[0], 10);
            newFruitPage.pageNum = extractedNumber;
          } else {
            console.log("No number found in the URL.");
          }
          newFruitPage.incomingUrls.push(currentUrl);
          await newFruitPage.save();
          fruitVisited.add(linkCompleteUrl);
          fruitCount++;
          fruitCrawler.queue(linkCompleteUrl);
        }

        // CREATING NEW DOC FOR SEED URL
        if ((currentUrl === fruitSeedUrl) && !fruitVisited.has(fruitSeedUrl)) {
          console.log("CREATING SEED PAGE");
          let newFruitPage = new FruitPage();
          newFruitPage.url = fruitSeedUrl;
          newFruitPage.incomingCount = 0;
          newFruitPage.outgoingCount = 0;
          newFruitPage.pageNum = 0;
          newFruitPage.title = title
          newFruitPage.body = body;
          fruitVisited.add(fruitSeedUrl);
          fruitCount++;
          await newFruitPage.save();
        }
  ...
  fruitCrawler.on('drain',async function() {
    if (fruitCount < FRUITMAX) {
      return;
    }
    wait(10).then(async () => {
      console.log("-----------------------------creating index for fruits-----------------------------")
      // update index
      let fruitPages = await FruitPage.find({});
      index_fruits = await createIndex();
      index_fruits = await indexPages(fruitPages, index_fruits);
      // calculate pageranks which is stored in index order AND update documents
      let pageRanks = await calculate(fruitPages, fruitCount);
      await fruitPages.forEach(page => {
        page.pageRank = pageRanks[page.pageNum];
        page.save();
      });
      console.log("-----------------------------Done crawling FRUITS for " + fruitCount + " pages-----------------------------");
      runPersonalCrawler();
    })
  });
  fruitCrawler.queue(fruitSeedUrl);
